...
object Main extends App {
  ...
  def dropDB = { //do a drop followed by initialisePeople
  ...
  def runQuery = {
    val insertPeople = Future {
      val query = peopleTable ++= Seq((10, "Jack", "Wood", 36), (20, "Tim", "Brown", 24)) // insert into `PEOPLE` (`PER_FNAME`,`PER_LNAME`,`PER_AGE`)  values (?,?,?)
  ...
  def updatePeople = {
    val updateFuture = Future{
      ...
      val updateAction = person.update("Curly Bois")
      db.run(updateAction)
    }
    Await.result(updateFuture,Duration.Inf).andThen {
      case Success(_) => listPeople
      case Failure(error) => println("updateing failed: " + error.getMessage)
    }
  }

  def listPeople = {
    val queryFuture = Future { // simple query that selects everything from People and prints them out
